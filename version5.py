...
basic_process = pd.read_csv("user_basic_info_processed.csv", names=['uid', 'gender', 'city', 'prodName', 'ramCapacity',
...
behavior_process = pd.read_csv("behavior_process.csv", names=['uid', 'bootTimes', 'CFuncTimes', 'DFuncTimes', 'FFuncSum'])
...
def mergeBasicTables(baseTable):
    resTable = baseTable.merge(basic_process, how='left', on='uid', suffixes=('_base0', '_ubaf'))
...
for index, (train_index, vali_index) in enumerate(skf.split(X, y)):
    ...
    model = lgb.train(param, train, num_boost_round=1100, valid_sets=[vali], early_stopping_rounds=50)
