...
def mergeBasicTables(baseTable):
    ...
    # cat_columns = ['city', 'prodName', 'carrier', 'os', 'ct']
    # resTable.drop(['ramCapacity', 'ramLeftRation', 'romCapacity', 'romLeftRation'], axis=1, inplace=True)
    # resTable.drop(['FFuncTimes'], axis=1, inplace=True)
...
def mergeAppData(baseTable):
    ...
    '''
    duration_max = resTable['duration_mean'].max()
    duration_min = resTable['duration_mean'].min()
    time_max = resTable['times_mean'].max()
    time_min = resTable['times_mean'].min()
    resTable["duration_mean"] = (resTable["duration_mean"] - duration_min) / (duration_max - duration_min)
    resTable["times_mean"] = (resTable["times_mean"] - time_min) / (time_max - time_min)
    '''
...
lda.fit(X1, y1)
X = lda.transform(X1)
test = PCA(n_components=10).fit_transform(test1)
'''
# In[7]:
